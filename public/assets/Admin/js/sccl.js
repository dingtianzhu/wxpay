...
$(".side-menu").on('click', 'li a', function(e) {
	...
	var l_parentli=$this.parent("li");
	//$this.parent("li").siblings("li").removeClass("active");
	//$this.parent("li").addClass("active");
	/*if(l_parentli!=null){
		var l_lis=l_parentli.siblings("li");
		if(l_lis.length>0)l_lis.removeClass("active");
		l_parentli.addClass("active");
	}*/
...
function addIframe(cur){
	...
	if (!isHas) {
		var tab = "<a href='javascript:;' class='content-tab active' data-id='"+h+"'><span class='cllabel'>"+label+"</span> <span class='glyphicon glyphicon-refresh' title='刷新本选项卡页'></span> <i class='icon-font'>&#xe617;</i></a>";
...
/*选项卡刷新事件*/
$(".tab-nav-content").on("click", ".content-tab span.glyphicon-refresh", refreshPage);
/*选项卡关闭事件*/
						...
						{"id":"12","name":"一级","parentId":"10","url":"","icon":"","order":"1","isHeader":"0","childMenus":[
							...
							{"id":"14","name":"二级","parentId":"12","url":"","icon":"","order":"1","isHeader":"0","childMenus":[
								...
								{"id":"16","name":"三级","parentId":"14","url":"","icon":"","order":"1","isHeader":"0","childMenus":[
									...
									{"id":"18","name":"四级","parentId":"16","url":"test1.html","icon":"","order":"1","isHeader":"0","childMenus":""}
								]}
							]}
						]}
					]}
...
//});
/*Long add 2017-3-12*/
function addAutoIframe(href,index,label){
	var h = href,
		m = index,
		isHas = false;
	if (h == "" || $.trim(h).length == 0) {
...
function adjustAutoIframe(href,label){
	var h = href;
	if (h == "" || $.trim(h).length == 0) {
		return false;
	}
	var prehref=$(".content-tab.active").data("id");
	//更新label
	//alert($(".content-tab.active").find(".cllabel").text());
	$(".content-tab.active").find("span.cllabel").text(label);
	//更新url
	$(".body-iframe").each(function() {
		if ($(this).data("id") == prehref) {
			$(this).attr("src",h);
		}
	});
	return false;
...
function closeFrameTab(name){
	$(".body-iframe").each(function() {
		if(this.name==name){
			var url=$(this).data("id");
			var tabs=$(".content-tab");
			for(var i=0;i<tabs.length;i++){
				if($(tabs[i]).data("id")==url){
					//alert(url);
					closePage.call($(tabs[i]).find('.icon-font')[0]);
					break;
				}
			}
		}
